...
def crawl_trending_coin():
    ...
    for coin in list_coin :
        ...
        market_cap = convert_float(coin.find_all("td")[7].get_text().replace("$", "").replace(",", ""))
        vol24h = convert_float(coin.find_all("td")[8].get_text().replace("$", "").replace(",", ""))
        if ud_24h == "down":
            h24h = convert_float(h24h.split('%')[0])* (-1)
        if ud_7d == "down":
            d7d = convert_float(d7d.split('%')[0])* (-1)
        if ud_30d == "down":
            d30d = convert_float(d30d.split('%')[0])* (-1)
